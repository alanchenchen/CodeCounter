<template>
  <div id="wrapper" ref="drag" class="drag">
    <Form :label-width="85" label-position="left">
      ...
      <FormItem>
        <div slot="label">
          <Tooltip content="目前仅支持同时统计一个文件和一个文件夹" placement="bottom-start">
              已添加
      ...
      showCommentRule() {
        ...
        list.forEach(a => {
          a[1].forEach((b, i) => {
            arr.push({
              type: `${a[0]}-${i}`,
              msg: `${a[0]} & ${b.block.start || ''}${b.block.end || ''} & ${b.row || ''}`
            })
          })
        })
        return arr
    ...
    methods: {
        ...
        if(hasVal) { 
          ...
          if(isSameExtNameAddRule) {
            const isRuleRepeat = Object.values(this.commentRule.list[key]).some(a => {
              return (filter[1].value && a.block.start == filter[1].value )
      ...
      codeCount() {
        ...
        counter(fileList, exclude, rule, (res, msg) => {
          const wasteTime = ((new Date().getTime() - startTime)/1000).toFixed(3)
